...
Metrics = namedtuple("Metrics", ["AUC", "ACC", "BALACC"])

# based on https://github.com/MedMNIST/MedMNIST/blob/main/medmnist/evaluator.py
...
class Evaluator:
    ...
    def evaluate(self, y_score, save_folder=None, run=None):
        ...
        balacc = getBALACC(self.labels, y_score, task)
        metrics = Metrics(auc, acc, balacc)
...
def getBALACC(y_true, y_score, task, threshold=0.5):
    ...
    if task == 'multi-label, binary-class':
        ...
        balacc = 0
        for label in range(y_true.shape[1]):
            label_balacc = balanced_accuracy_score(y_true[:, label], y_pre[:, label])
            balacc += label_balacc
        ...
        if y_score.ndim == 2:
            ...
            assert y_score.ndim == 1
        ret = balanced_accuracy_score(y_true, y_score > threshold)
    else:
        ret = balanced_accuracy_score(y_true, np.argmax(y_score, axis=-1))
